...
export default function CotizadorProfesional() {
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f8fafc', paddingBottom: '2rem' }}>
      {/* SECCIÓN DE IMPRESIÓN ÚNICA */}
      <div id="cotizacion-pdf" style={{ maxWidth: '1000px', margin: '0 auto', backgroundColor: 'white', boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }}>
                ...
                <tr key={index} style={{ backgroundColor: index % 2 === 0 ? 'white' : '#f8fafc' }}>
                  ...
                  <td style={{ textAlign: 'center', border: '1px solid #e2e8f0', fontWeight: 'bold', color: '#1e3a5f' }}>
                    <div className="screen-only" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.4rem' }}>
                      ...
                      <button onClick={() => eliminarItem(index)} style={{ color: '#ef4444', border: 'none', background: 'none' }}>✕</button>
                    </div>
        ...
        {/* Totales y Cuentas */}
        <div id="print-stopper" style={{ padding: '1rem 1.5rem', background: '#f8fafc', borderTop: '2.5px solid #1e3a5f' }}>
          ...
          <p style={{ textAlign: 'right', fontSize: '0.75rem', fontStyle: 'italic', margin: '0 0 1rem 0' }}>⏰ Válido por 7 días</p>
          <div className="cuentas-bancarias" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.8rem', borderTop: '1.5px solid #e2e8f0', paddingTop: '0.8rem' }}>
            ...
            <div style={{ fontSize: '0.85rem' }}><b>Yape:</b><br/><span style={{ fontSize: '1.05rem', fontWeight: 'bold' }}>964194540</span></div>
          </div>
        </div>
        ...
        <div className="screen-only" style={{ padding: '1.5rem', backgroundColor: '#f1f5f9', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.8rem' }}>
          <button onClick={agregarItem} style={{ padding: '0.8rem', backgroundColor: 'white', color: '#2d5a8c', border: '2px solid #2d5a8c', fontWeight: 'bold', borderRadius: '0.4rem' }}>+ Agregar producto</button>
          <button onClick={guardarPDF} style={{ padding: '0.8rem', background: 'linear-gradient(135deg, #059669 0%, #047857 100%)', color: 'white', border: 'none', fontWeight: 'bold', borderRadius: '0.4rem' }}>💾 Guardar PDF / Historial</button>
          <button onClick={nuevaCotizacion} style={{ padding: '0.8rem', backgroundColor: '#64748b', color: 'white', border: 'none', borderRadius: '0.4rem' }}>🔄 Nueva</button>
          <button onClick={() => { if(confirm('¿Limpiar historial?')) { localStorage.removeItem('historial_rodrigo'); setHistorial([]); }}} style={{ padding: '0.7rem', backgroundColor: '#ef4444', color: 'white', borderRadius: '0.4rem' }}>🗑️ Limpiar Historial</button>
        </div>
      </div>

      {/* Historial Pantalla */}
      <div className="screen-only no-print-ever" style={{ maxWidth: '1000px', margin: '2rem auto', padding: '1.5rem', backgroundColor: 'white' }}>
      ...
      <style dangerouslySetInnerHTML={{__html: `
        @media print {
          ...
          html, body { height: 100%; overflow: hidden !important; margin: 0 !important; padding: 0 !important; }
          .screen-only, .no-print-ever { display: none !important; visibility: hidden !important; height: 0 !important; position: absolute !important; pointer-events: none !important; }
          .print-only { display: block !important; visibility: visible !important; }
          .contenedor-principal { box-shadow: none !important; margin: 0 !important; width: 100% !important; max-width: none !important; height: auto !important; padding: 1.5cm !important; }
          .header-diseno { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
          #print-stopper { page-break-after: avoid !important; }
        }
